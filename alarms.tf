############################
# Targets: instances to watch
############################
data "aws_instances" "targets" {
  filter {
    name   = "tag:${var.target_tag_key}" # e.g., Monitor
    values = [var.target_tag_value]      # e.g., Enabled
  }
  filter {
...
############################
# Per-path DISK alarms (CWAgent)
############################
# Build (instance_id, path) pairs
locals {
...
resource "aws_cloudwatch_metric_alarm" "disk_usage_high" {
  ...
  alarm_description  = "Disk usage > ${var.fs_usage_threshold}% for 5 min on ${each.value.instance_id} (path ${each.value.path})"
...
resource "aws_cloudwatch_metric_alarm" "cpu_high" {
  ...
  alarm_actions = [aws_sns_topic.opsgenie.arn]
  ok_actions    = [aws_sns_topic.opsgenie.arn] # optional
}

############################
# Memory alarm (CWAgent)
############################
# Requires mem metrics in your cwagent_config.json
# (mem_used_percent collected by the CloudWatch Agent)
